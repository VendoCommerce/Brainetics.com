...
namespace CSWeb.PS_K2.UserControls
{
    public partial class ShippingForm : System.Web.UI.UserControl
    {
        ...
        protected void imgBtn_OnClick(object sender, ImageClickEventArgs e)
        {
            if (!validateInput())
            {
                SaveData();
                string pid = "64";
                Response.Redirect(string.Format("AddProduct.aspx?PId={0}&CId={1}",pid, Convert.ToString((int)CSBusiness.ShoppingManagement.ShoppingCartType.SingleCheckout)));
